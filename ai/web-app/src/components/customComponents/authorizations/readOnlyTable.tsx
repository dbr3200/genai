...
interface Props {
   ...
   */
  readOnlyData: Array<Record<string, any>>;
  /**
...
export const ReadOnlyTable = ({
    ...
    return (
      <PerfectScrollBar component="div" className={styles.psContainer}>
        <Table classes={styles.authTable}>
          <Table.Body>
            {searchFunction( data )?.length > 0 ? searchFunction( data )
                    ...
                    <td className="group">
                      <div className={styles.listItem}>
                        ...
                        <div className={clsx( styles.ctas, "invisible group-hover:visible" )}>
                          <Tooltip
                            trigger={<Button title={t( "authorization.removeUser" )} aria-label={t( "authorization.removeUser" )}
                              variant="icon"
                              // disabled={isUsersUpdating || user?.UserId === username}
                              size="sm" onClick={() => setSelectedUser( user?.UserId )}
                              icon={
                                <ADPIcon size="sm" icon="delete-user" classes="text-danger/50 hover:text-danger/80" />
                              } />}>
        ...
        {data?.length === 0 && <EmptyState display="vertical" transparentBG>
          <EmptyState.Content title={`${t( "authorization.noReadOnly" )} ${t( `authorization.${serviceName}` ).toLocaleLowerCase()}`} />
        </EmptyState>}
      </PerfectScrollBar>
    );
  }
  ...
  }, [ readOnlyData, isUsersUpdating, updateAccess, serviceName, resourceId, showSuccessNotification ]);
